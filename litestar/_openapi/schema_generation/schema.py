...
def create_schema_for_struct_class(
    ...
    return Schema(
        required=sorted(
            [
                struct.name
                for struct in msgspec_struct_fields(field_type)
                if struct.default is NODEFAULT
                and struct.default_factory is NODEFAULT
                and not is_optional_union(field_type_hints[struct.name])
            ]
        ),
        properties={
            k: create_schema(
